...
- **Agent Logger**: AI agent operations and function calls
- **Analyzer Logger**: Data analysis operations
- **UI Logger**: User interface interactions
- **App Logger**: Main application events
...
3. **Memory Issues**: Large CSV files may require more memory
4. **Plot Display Issues**: Check if matplotlib backend is properly configured

### Debug Mode

Enable debug logging by modifying the log level in `src/utils/logger.py`:
```python
